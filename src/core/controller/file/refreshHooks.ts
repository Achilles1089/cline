...
export async function refreshHooks(
	...
	const globalHooksDir = globalHooksDirOverride || path.join(os.homedir(), "Documents", "Dappit AI", "Hooks")
	...
	for (const workspacePath of workspacePaths.paths) {
		const workspaceHooksDir = path.join(workspacePath, ".dappitrules", "hooks")
