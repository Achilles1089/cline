	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Dappit Rules & Workflows</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Dappit Rules & Workflows" : "Show Dappit Rules & Workflows"}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-2 pt-0">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Dappit AI with system-level guidance. Think of them as a persistent way
								...
								<p>
									Workflows allow you to define a series of steps to guide Dappit AI through a repetitive set of
								...
								<p>
									Skills are reusable instruction sets that Dappit AI can activate on-demand. When a task matches a
									skill's description, Dappit AI uses the <span className="font-bold">use_skill</span> tool to load
								...
								{workspaceHooks.map((workspace, index) => (
									<div
										...
										<div className="text-sm font-normal mb-2">
											{workspace.workspaceName}/.dappitrules/hooks/
										</div>
										<div className="flex flex-col gap-0">
											{workspace.hooks
												.sort((a, b) => a.name.localeCompare(b.name))
